    ...
    class Display
      ...
      class Unknown
        ...
        def announce(context)
          LOGGER.debug(DISPLAY_UNKNOWN) { '#announce' }
          context.change_state(Enabled.new)
        end

        def monitor_on(context)
          LOGGER.debug(DISPLAY_UNKNOWN) { '#monitor_on' }
          context.change_state(Enabled.new)
        end

        def monitor_off(context)
          LOGGER.debug(DISPLAY_UNKNOWN) { '#monitor_off' }
          context.change_state(Off.new)
        end

        def input_menu(context)
          LOGGER.debug(DISPLAY_UNKNOWN) { '#input_menu' }
          context.change_state(Enabled.new)
        end

        def kl_30(context)
          LOGGER.debug(DISPLAY_UNKNOWN) { '#kl_30' }
        ...
        def code_on(context)
          LOGGER.debug(DISPLAY_UNKNOWN) { '#code_on' }
          context.change_state(Busy::Code.new)
        end

        def prog_on(context)
          LOGGER.debug(DISPLAY_UNKNOWN) { '#prog_on' }
