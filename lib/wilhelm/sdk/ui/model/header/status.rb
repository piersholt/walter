      ...
      module Header
        ...
        class Status
          ...
          def devices_update(action, device:)
            LogActually.ui.debug(self.class.name) { "#devices_update(#{action}, #{device})" }
            case action
            when :connected
              devices(device)
            when :disconnected
            ...
            when :created
              ...
              LogActually.ui.debug(self.class.name) { "devices_update: #{action} not implemented." }
            end
          end

          def player_update(action, player:)
            LogActually.ui.debug(self.class.name) { "#player_update(#{action}, #{player})" }
            case action
            ...
            when :status
              ...
              LogActually.ui.debug(self.class.name) { "player_update: #{action} not implemented." }
            end
          end
