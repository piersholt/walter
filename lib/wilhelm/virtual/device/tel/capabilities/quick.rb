        ...
        module Capabilities
          ...
          module Quick
            ...
            def quick_name(index = 0, text = QUICK_NAME)
              ...
              draw_23(to: :ike, gt: 0x42, ike: 0x20, chars: formatted_text)
            end

            def quick_number(index = 0, text = QUICK_NUMBER)
              formatted_text = format_quick_contact(index, text)
              draw_23(to: :ike, gt: 0x43, ike: 0x10, chars: formatted_text)
            end

            def quick_call_start(text = QUICK_NUMBER)
              formatted_text = format_quick_dial(text)
              draw_23(to: :ike, gt: 0x43, ike: 0x20, chars: formatted_text)
            end

            def quick_call_end
              draw_23(to: :ike, gt: 0x40, ike: 0x20, chars: STRING_BLANK)
            end

            def quick_exit
              draw_23(to: :ike, gt: 0x41, ike: 0x20, chars: STRING_BLANK)
            end

            def quick_error(text = QUICK_ERROR)
              draw_23(to: :ike, gt: 0x40, ike: 0x20, chars: text)
            end
