  ...
  module Core
    module DataLink
      module Synchronisation
        # Frame synchronisation for Diagnostic (D) bus.
        class Diagnostic
          include Constants
          include Errors
          ...
          def fetch_frame_header
            ...
            LOGGER.debug(name) { "#{SYNC_HEADER} Shifting #{DS2_HEADER_LENGTH} bytes." }
            header = buffer.shift(DS2_HEADER_LENGTH)
          ...
          def read_length_from_header
            ...
            LOGGER.debug(name) { "#{SYNC_HEADER} Remaining frame bytes: #{frame.header.tail_length} - #{DS2_HEADER_LENGTH} => #{frame.header.tail_length - DS2_HEADER_LENGTH}" }
            frame.header.tail_length - DS2_HEADER_LENGTH
          rescue HeaderValidationError, HeaderImplausibleError, TailValidationError, ChecksumError => e
            # Frame errors must be rescued to mitigate rescue StandardError
            raise e
          rescue StandardError => e
